"""Phase 3 — both models, mixed workloads.

Benchmarks Qwen2.5-VL-7B and InternVL2.5-8B preprocessing on two new workloads:
  W3: 32 images with random sizes in [512, 2048] and aspect ratio in [0.5, 2.0].
...
Outputs (one per processor variant × workload):
  - profiles/Q25_W3_legacy.txt, Q25_W3_fast.txt
  - profiles/Q25_W4_legacy.txt, Q25_W4_fast.txt
  - profiles/IV25_W3_hf_legacy.txt, IV25_W3_hf_fast.txt, IV25_W3_manual.txt
  - profiles/IV25_W4_hf_legacy.txt, IV25_W4_hf_fast.txt, IV25_W4_manual.txt
  - stdout summary per workload with timing, peak/output ratio, env info.
...
from models import load_processors, get_internvl_hf_processor, get_internvl_manual_processor
...
def run_workload(label, images, pdir, q_slow, q_fast, iv_hf_slow, iv_hf_fast, iv_manual,
                 n_warmup, n_timed):
    n = len(images)
    tag = label.upper()

    print(f"\n{'='*60}")
    print(f"workload {label}: {n} images")
    print(f"{'='*60}")
...
def main():
    ap = argparse.ArgumentParser(description="Phase 3: both models on W3 and W4 workloads.")
    ...
    iv_hf_slow = get_internvl_hf_processor(use_fast=False)
    iv_hf_fast = get_internvl_hf_processor(use_fast=True)
    ...
    run_workload("W3", images_w3, pdir, q_slow, q_fast, iv_hf_slow, iv_hf_fast, iv_manual,
                 args.n_warmup, args.n_timed)
    run_workload("W4", images_w4, pdir, q_slow, q_fast, iv_hf_slow, iv_hf_fast, iv_manual,
                 args.n_warmup, args.n_timed)
