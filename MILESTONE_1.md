...
We observe for the large, uniform W4 images, Intern and LlaVA spend a large fraction of the runtime spent on pure resizing. For these particular cases, a DSL may not be needed. However, this still supports the need for a scheduling abstraction: the best optimization strategy changes across model families and image regimes. A DSL provides the most flexibility for each case. The benchmarking we have completed helps us idenitify where a scheduling DSL would help the most. 